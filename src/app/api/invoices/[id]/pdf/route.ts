...
function statusLabel(s: string): string {
  ...
  if (s === "PARTIAL") return "CICILAN (PARTIAL)";
  return "BELUM BAYAR";
}
function statusColor(s: string): [number, number, number] {
  if (s === "PAID") return [16, 185, 129];
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [sr, sg, sb] = statusColor(invoice.status);
  ...
  doc.text(formatDate(invoice.dueDate), margin + 55, y);
  doc.text(formatCurrency(invoice.totalAmount), margin + 110, y);
  y += 2;
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(15, 23, 42);
  const clientName = client?.name ?? `Klien #${invoice.clientId}`;
  doc.text(clientName, leftX, y);
  y += 5;
  doc.setFontSize(8);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(71, 85, 105);
  if (client?.contactInfo) { doc.text(`Kontak: ${client.contactInfo}`, leftX, y); y += 4; }
  if (client?.websiteUrl) { doc.text(`Website: ${client.websiteUrl}`, leftX, y); y += 4; }
  if (!client?.contactInfo && !client?.websiteUrl) { doc.text("—", leftX, y); y += 4; }
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(15, 23, 42);
  const serviceName = service?.name ?? `Layanan #${invoice.serviceId}`;
  ...
  if (service?.description) {
    const descLines = doc.splitTextToSize(service.description, colW);
    doc.text(descLines.slice(0, 3), rightX, ry);
    ry += Math.min(descLines.length, 3) * 4;
  }
  doc.setTextColor(100, 116, 139);
  doc.setFontSize(7);
  doc.text(`Harga paket: ${formatCurrency(service?.basePrice ?? invoice.totalAmount)}`, rightX, ry);
  ry += 4;
  ...
  const boxes: Array<{ label: string; value: string; color: [number, number, number] }> = [
    { label: "TOTAL TAGIHAN", value: formatCurrency(invoice.totalAmount), color: [15, 23, 42] },
  ...
  boxes.forEach((b, i) => {
    ...
    const isLast = i === 2;
    if (isLast && remaining === 0) { doc.setFillColor(236, 253, 245); doc.setDrawColor(167, 243, 208); }
    else if (isLast && remaining > 0) { doc.setFillColor(255, 241, 242); doc.setDrawColor(254, 205, 211); }
    else if (i === 1) { doc.setFillColor(236, 253, 245); doc.setDrawColor(167, 243, 208); }
  ...
  y += boxH + 8;
  const progress = invoice.totalAmount > 0 ? Math.round((paidTotal / invoice.totalAmount) * 100) : 0;
  doc.setFontSize(7);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(100, 116, 139);
  ...
  if (progress > 0) { doc.setFillColor(16, 185, 129); doc.roundedRect(margin, y, (barW * progress) / 100, 3, 1.5, 1.5, "F"); }
  y += 8;
  doc.setFontSize(9);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(15, 23, 42);
  ...
  doc.setFont("helvetica", "normal");
  doc.setTextColor(100, 116, 139);
  doc.text(`${paymentRows.length} transaksi`, margin, y);
  y += 5;
  if (paymentRows.length === 0) {
    ...
    paymentRows.forEach((p, idx) => {
      if (y > pageHeight - 22) { doc.addPage(); y = 14; }
      const rowH = 7;
      if (idx % 2 === 0) { doc.setFillColor(249, 250, 251); doc.rect(margin, y, pageWidth - margin * 2, rowH, "F"); }
  ...
  doc.text(`Dokumen dibuat otomatis oleh OkeSite CRM  •  ${new Date().toLocaleDateString("id-ID", { day: "2-digit", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" })}  •  ${invoiceNumber}`, pageWidth / 2, footerY, { align: "center" });
  doc.setFontSize(6);
  doc.text("Terima kasih atas kepercayaan Anda menggunakan layanan OkeSite.", pageWidth / 2, footerY + 4, { align: "center" });

  const arrayBuffer = doc.output("arraybuffer") as ArrayBuffer;
  const filename = `${invoiceNumber}-${(client?.name || "klien").replace(/\s+/g, "-").toLowerCase().slice(0, 20)}.pdf`;
